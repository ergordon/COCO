...
with open(os.path.join(os.getcwd(),'SubImages.csv'),'w') as csvfile:
    ...
    wrtr.writerow(["SubImage Number","Class Name","RelativeOriginX2Absolute","RelativeOriginY2Absolute","Ymin","Xmin","Ymax","Xmax"])
        ...
        for y in range(0,width,N):
            ...
            for c in range(0, len(classes)):
                if scores[c] > threshold:
                    ...
                    wrtr.writerow(["subImage_"+str(len(tiles)),str(class_name),int(x),int(y), boxes[c,0], boxes[c,1], boxes[c,2], boxes[c,3]])
                    csvfile.flush()
