...
ap.add_argument("-i", "--DataA", type=str, required=True)
ap.add_argument("-o", "--DataB", type=str, required=True)
...
A_HtW = []
A_z=0
with open(os.path.join(path ,args["DataA"]), 'r') as f:
...
B_HtW = []
B_z=0
with open(os.path.join(path ,args["DataB"]), 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        try:
            B_z = B_z+1
