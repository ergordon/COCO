...
def lightIntenseFunc (str, path):
    ...
    with open(os.path.join(path,'holes.csv'), mode='a') as csvfile:
        ...
        for (i, c) in enumerate(cnts):
            ...
            conv = 175 # Conversion 175px per 1mm       
            
            filewriter.writerow([str, int(cX)/conv, int(cY)/conv, int(radius)/conv, xRel + int(cX)/conv, yRel + int(cY)/conv, int(lMean)])
